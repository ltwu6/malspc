...
from utils.print_util import *
from utils.time_util import *
from utils.data_util import remove_store
import torch
import torch.optim as optim
from torch.utils.data import Dataset, DataLoader
from utils.data_util import depthToint16
...
class TrainStatus():
    ...
    def print(self, curr_epoch, curr_train, curr_valid, go_back=True):
        ...
        print_line(text)
        print_line('LAST SAVE PATH: '+str(self.last_save_path))
        print_line(('LAST %d TRAIN LOG:' % self.train_log_num) + (' None' if len(self.train_log)==0 else ''))
        num = 4
        for log in self.train_log:
            print_line(log)
            num += 1
        print_line('LAST VALID LOG:' + (' None' if len(self.valid_log)==0 else ''))
        num += 1
...
class TrainFramework():
    def __init__(self, batch_size, log_dir, is_restore, cuda_index):
        ...
        self.res_dir = os.path.join(self.log_dir)
        self.best_dir = os.path.join(self.res_dir,'best')
    ...
    def _set_dataset(self, dataset_name, train_dataset, valid_dataset):
        self.train_dataset_name = dataset_name
        self.valid_dataset_name = dataset_name
        ...
        self.num_work = 4
        self.train_loader = DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle=True, num_workers=self.num_work,drop_last=True)
        self.valid_loader = DataLoader(self.valid_dataset, batch_size=1, shuffle=False, num_workers=self.num_work)
    ...
    def _set_optimzer(self, optimizer, **kwargs):
        if optimizer == 'Adam':
            ...
            if 'G_freeze' in kwargs.keys() and kwargs['G_freeze']:
                self.G_freeze = True
                self.optimizer_G = None 
                for p in self.net.G.parameters():
                    p.requires_grad = False
            else:
                self.G_freeze = False
                self.optimizer_G = torch.optim.Adam(self.net.G.parameters(), lr=self.lr, weight_decay=self.weight_decay)
            
            if 'D_freeze' in kwargs.keys() and kwargs['D_freeze']:
                self.D_freeze = True
                self.optimizer_D = None
                for p in self.net.D.parameters():
                    p.requires_grad = False
            else:
                self.D_freeze = False
                self.optimizer_D = torch.optim.Adam(self.net.D.parameters(), lr=self.lr, weight_decay=self.weight_decay)
    ...
    def train(self, epoch, G_opt_step=1, D_opt_step=1, print_pre_step=100, \
        test_pre_step=-1, save_pre_epoch=10, last_epoch=None, \
        after_restore_func=None, after_valid_func=None, on_save_func=None, \
        restore_func=None, epoch_test=True):
        '''
        ...
        self.total_tqdm_num = int((epoch+1)*(self.train_step + self.test_num_pre_epoch*self.valid_step))

        scheduler_g = torch.optim.lr_scheduler.StepLR(self.optimizer_G, step_size = 200, gamma = 0.5, last_epoch=-1)       
        scheduler_d = torch.optim.lr_scheduler.StepLR(self.optimizer_D, step_size = 200, gamma = 0.5, last_epoch=-1)
        ...
        for epoch in range(start_epoch, self.epoch+1):
            ...
            for i, data in enumerate(self.train_loader):
                ...
                depth, part, mask, param, ids = data
                depth = depth.to(device=self.device)
                part = part.to(device=self.device)
                mask = mask.to(device=self.device)
                param = param.to(device=self.device)
                
                outputs = self.net(part,depth,param)
                loss = self.net.loss(outputs, mask, part)
                mean_loss += np.array([l.item() for l in loss])
        
                if (not self.G_freeze) and (self.curr_step % G_opt_step == 0):
                    self.optimizer_G.zero_grad()
                    loss[0].backward(retain_graph=True)

                if (not self.D_freeze) and (self.curr_step % D_opt_step == 0):
            ...
            if self.epoch_test:
                ...
                for j, data in enumerate(self.valid_loader):
                    ...
                    depth_test, param_test, part_test, gt_test, ids_test = data
                    info = ids_test[0].split('/')[0]
                    depth_test = depth_test.to(device=self.device)
                    param_test = param_test.to(device=self.device)
                    part_test = part_test.to(device=self.device)
                    gt_test = gt_test.to(device=self.device)
                    
                    with torch.no_grad():
                        coarse, fine, feat, sil_map_c, sil_map_f, depth_f, _, _, _, _, _ = self.net(part_test, depth_test,param_test)
                        loss = self.net.loss_test(fine, gt_test)
                        # print('shape of loss: ', loss[0].shape)
                        # save_path = os.path.join(self.res_dir,'pc',info)
                            
                    for bi, inf in enumerate([info]):
                        if inf not in loss_dict:
                            loss_dict[inf] = [np.zeros([self.net.loss_test.loss_num]), 0]
                        # print('.......',np.array([l for l in loss]))
                        loss_dict[inf][0] += np.array([l.item() for l in loss])
                        # print('shape of loss_dict[inf][0]: ', loss_dict[inf][0].shape)
                        loss_dict[inf][1] += 1
                    t.update()
                ...
                avg_loss /= avg_num
                cd_avg = avg_loss[0]*1e4
                if cd_avg<best_cd:
                    if os.path.exists(os.path.join(self.log_dir, 'model-%d.pkl' % best_epoch)):
                        os.remove(os.path.join(self.log_dir, 'model-%d.pkl' % best_epoch))
                    best_cd = cd_avg
                    # coarse_cd = avg_loss[1]*1e4
                    best_epoch = epoch
            ...
            print('best cd and epoch: ', np.round(float(best_cd),5), best_epoch)
...
def train(args):
    ...
    from utils.dataloader import TestDataLoader, TrainDataLoader

    from models.model_cfgan import OUR
    train_dataset = TrainDataLoader(args.list_dir, args.data_path, args.input_pnum, args.category)
    valid_dataset = TestDataLoader(args.list_dir, args.data_path, status='test', pc_input_num=args.input_pnum, category=args.category)

    net = OUR()

    tf = TrainFramework(args.batch_size, args.log_dir, args.restore, cuda_index)
    tf._set_dataset(args.data_name, train_dataset, valid_dataset)
    tf._set_net(net, 'Base')
    tf._set_optimzer(args.optimizer, lr=args.lr, weight_decay=args.weight_decay)
    tf.train(args.max_epoch, G_opt_step=1, D_opt_step=1, save_pre_epoch=10, print_pre_step=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--list_dir', default='./model_list')
    parser.add_argument('--data_path', default='')
    parser.add_argument('--category', default='plane')
    parser.add_argument('--use_cuda', default=True)
    parser.add_argument('--cuda_index', default='6')
    parser.add_argument('--data_name', default='data_name')
    # parser.add_argument('--class_name', default='plane', choices=['plane','cabinet','car','chair','lamp','couch','table','watercraft'])
    ...
    parser.add_argument('--input_pnum', type=int, default=2048)
    parser.add_argument('--gt_pn', type=int, default=16384)
    parser.add_argument('--max_epoch', type=int, default=600)
    
    args = parser.parse_args()
    args.log_dir = args.log_dir + args.category
    train(args)