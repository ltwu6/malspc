...
from pytorch3d.renderer import rasterize_points
# from .rasterize_points import rasterize_points
from models.utils import DGCNN
...
class PointsRasterizer_Learn(nn.Module):
    """
    This class implements methods for rasterizing a batch of pointclouds.
    """

    def __init__(self, cameras=None, raster_settings=None, upb=0.03,lwb=0.01) -> None:
        """
        ...
        self.raius_learning = nn.Sequential(
            nn.Conv1d(3, 128, 1),
            nn.BatchNorm1d(128),
        ...
        self.upper_bound = upb
        self.lower_bound = lwb
    ...
    def forward(self, point_clouds, **kwargs) -> PointFragments:
        ...
        # print('shape of point proj: ', points_proj._points_padded.shape) # [32, 16384, 3]
        # print('type of point proj: ', type(points_proj._points_padded)) # [32, 16384, 3]
        l_radius = self.raius_learning(points_proj._points_padded.permute(0,2,1))
        l_radius = self.sigm(l_radius)*(self.upper_bound-self.lower_bound)+self.lower_bound
        # print('shape of l radius: ', l_radius.shape)
        raster_settings = kwargs.get("raster_settings", self.raster_settings)
        idx, zbuf, dists2 = rasterize_points(
            points_proj,
            image_size=raster_settings.image_size,
            radius=l_radius.squeeze(1),
...
class PointsRasterizer_Transform(nn.Module):
    """
    This class implements methods for rasterizing a batch of pointclouds.
    """
...
class PointsRasterizer_RasterizePoints(nn.Module):
    """
    This class implements methods for rasterizing a batch of pointclouds.
    """
    ...
    def forward(self, points_proj, l_radius, **kwargs) -> PointFragments:
        """
        Args:
            point_clouds: a set of point clouds with coordinates in world space.
        Returns:
...
class PointsRasterizer_AdaRad(nn.Module):
    """
    This class implements methods for rasterizing a batch of pointclouds.
    """
    ...
    def forward(self, point_clouds, l_radius, **kwargs) -> PointFragments:
        """
        Args:
            point_clouds: a set of point clouds with coordinates in world space.
        Returns:
